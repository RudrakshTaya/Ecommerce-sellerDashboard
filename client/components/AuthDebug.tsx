...
const AuthDebug: React.FC = () => {
    ...
    try {
      ...
      setDebug((prev) => ({
        ...
        products: response.success
          ? `Products fetch SUCCESS (${response.data?.length || 0} items)`
  ...
  const testProtectedEndpoint = async () => {
    try {
      ...
      const response = await fetch(
        createApiUrl(API_CONFIG.ENDPOINTS.TEST.PROTECTED),
        {
          headers: {
      ...
      setDebug((prev) => ({
        ...prev,
        protectedTest: response.ok
          ? "Protected endpoint SUCCESS"
          : `Protected endpoint FAILED: ${data.message}`,
        protectedData: data,
      }));
    } catch (error) {
      setDebug((prev) => ({
  ...
  const testPublicEndpoint = async () => {
    try {
      const response = await fetch(
        createApiUrl(API_CONFIG.ENDPOINTS.TEST.PUBLIC),
      );
      const data = await response.json();

      setDebug((prev) => ({
        ...prev,
        publicTest: response.ok
  ...
  return (
    <div className="p-4 bg-gray-100 rounded-lg">
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <Button onClick={testToken} size="sm">
          Check Token
        </Button>
        <Button onClick={testPublicEndpoint} size="sm">
          Test Public API
        </Button>
        <Button onClick={testProtectedEndpoint} size="sm">
          Test Protected API
        </Button>
        <Button onClick={testProfile} size="sm">
          Test Profile API
        </Button>
        <Button onClick={testProducts} size="sm">
          Test Products API
        </Button>
        <Button onClick={() => setDebug({})} size="sm" variant="outline">
          Clear
        </Button>
      </div>
