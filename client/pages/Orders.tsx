...
const OrderDetailsModal = ({
  ...
  const sellerItems = order.items.filter(
    (item) => item.sellerId === order.items[0]?.sellerId,
  );
              ...
              <CardContent className="p-6">
                <div className="space-y-4">
                  {sellerItems.map((item, index) => (
                    <div
                      key={index}
                      className="flex items-start space-x-4 p-4 border-2 border-gray-100 rounded-lg bg-gray-50/50"
                    >
                      <img
      ...
      <CardContent className="space-y-4 p-4">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Select
            value={order.status}
            onValueChange={(value) =>
              onStatusUpdate(order.id, value as Order["status"])
            }
          >
