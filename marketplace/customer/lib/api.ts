...
export const authAPI = {
  ...
  getCurrentCustomer: async (): Promise<Customer> => {
    const response: AxiosResponse<{ success: boolean; customer: Customer }> =
...
export const productsAPI = {
  getProducts: async (
    ...
    const response: AxiosResponse<PaginatedResponse<Product>> = await api.get(
      "/public/products",
      {
        params,
      },
  ...
  getProductsBySeller: async (
    ...
    const response: AxiosResponse<PaginatedResponse<Product>> = await api.get(
      `/public/products/seller/${sellerId}`,
      {
        params,
      },
    );
...
export const categoriesAPI = {
  ...
  getCategory: async (slug: string): Promise<Category> => {
    const response: AxiosResponse<ApiResponse<Category>> = await api.get(
...
export const sellersAPI = {
  getSellers: async (): Promise<Seller[]> => {
    const response: AxiosResponse<ApiResponse<Seller[]>> =
      await api.get("/public/sellers");
    return response.data.data!;
  },

  getSeller: async (id: string): Promise<Seller> => {
    const response: AxiosResponse<ApiResponse<Seller>> = await api.get(
      `/public/sellers/${id}`,
    );
    return response.data.data!;
  },
...
export const ordersAPI = {
  ...
  trackOrder: async (
    id: string,
  ): Promise<{ status: string; tracking?: any }> => {
...
export const addressesAPI = {
  ...
  addAddress: async (address: Omit<Address, "_id">): Promise<Address> => {
    const response: AxiosResponse<ApiResponse<Address>> = await api.post(
      "/customer-auth/addresses",
      address,
    );
    return response.data.data!;
  ...
  updateAddress: async (
    ...
    const response: AxiosResponse<ApiResponse<Address>> = await api.put(
      `/customer-auth/addresses/${id}`,
      address,
    );
    return response.data.data!;
  },
  ...
  setDefaultAddress: async (id: string): Promise<Address> => {
    const response: AxiosResponse<ApiResponse<Address>> = await api.put(
      `/customer-auth/addresses/${id}/default`,
    );
    return response.data.data!;
  },
};
