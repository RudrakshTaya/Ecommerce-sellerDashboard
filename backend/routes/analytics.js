...
router.get('/dashboard', protect, asyncHandler(async (req, res) => {
  try {
    ...
    const [
      ...
      Order.aggregate([
        { $match: { sellerId, status: 'delivered', createdAt: { $gte: last30Days } } },
        { $group: { _id: null, total: { $sum: '$total' } } }
      ]),
      Order.aggregate([
        { $match: { sellerId, status: 'delivered', createdAt: { $gte: last7Days } } },
        { $group: { _id: null, total: { $sum: '$total' } } }
      ]),
...
router.get('/sales', protect, [
  ...
  try {
    ...
    const matchQuery = { sellerId: req.seller._id, status: 'delivered', ...dateFilter };
...
router.get('/products', protect, asyncHandler(async (req, res) => {
  try {
    ...
    const poorPerformers = productMetrics
      .filter(product => product.totalSold === 0 && product.status === 'active')
      .slice(0, 10);
...
// @access  Private
router.get('/customers', protect, asyncHandler(async (req, res) => {
  try {
    const sellerId = req.seller._id;
    ...
    const atRiskCustomers = customerAnalytics.filter(
      customer => customer.lastOrder < sixtyDaysAgo && customer.totalOrders > 1
    );
    ...
    const averageOrderValue = customerAnalytics.reduce((sum, customer) => 
      sum + customer.averageOrderValue, 0) / totalCustomers || 0;
    const averageLifetimeValue = customerAnalytics.reduce((sum, customer) => 
      sum + customer.totalSpent, 0) / totalCustomers || 0;
