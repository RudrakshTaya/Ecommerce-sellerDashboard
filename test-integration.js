...
async function runTests() {
  ...
  console.log(search.success ? '✅ Search API working' : '❌ Search API failed');

  // 4. Test categories
  console.log('\n📂 Testing Categories API...');
  const categories = await testAPI('/public/categories');
  tests.push({ name: 'Categories API', result: categories });
  console.log(categories.success ? '✅ Categories API working' : '❌ Categories API failed');

  // 5. Test mock data
  console.log('\n🎭 Testing Mock Data...');
  const mockProducts = await testAPI('/mock/products');
  tests.push({ name: 'Mock Products', result: mockProducts });
  console.log(mockProducts.success ? '✅ Mock data working' : '❌ Mock data failed');
  ...
  console.log(customerReg.success ? '✅ Customer registration working' : '❌ Customer registration failed');
  ...
  console.log(sellerReg.success ? '✅ Seller registration working' : '❌ Seller registration failed');
  ...
  console.log(analytics.status === 401 ? '✅ Analytics endpoint exists (auth required)' : '❌ Analytics endpoint failed');
  ...
  console.log(payment.status === 401 ? '✅ Payment endpoint exists (auth required)' : '❌ Payment endpoint failed');
  ...
  console.log(notifications.status === 401 ? '✅ Notifications endpoint exists (auth required)' : '❌ Notifications endpoint failed');
  ...
  const passed = tests.filter(t => t.result.success || t.result.status === 401).length;
  const total = tests.length;

  tests.forEach(test => {
    const status = test.result.success ? '✅' : 
                  test.result.status === 401 ? '🔒' : '❌';
    const message = test.result.success ? 'PASS' :
                   test.result.status === 401 ? 'AUTH_REQUIRED' : 'FAIL';
  ...
  if (passed === total) {
    ...
    console.log(`⚠️ ${total - passed} tests failed - some features may need attention`);
  }
...
async function checkMarketplace() {
  try {
    ...
    console.log('🛍️ Marketplace frontend: ' + (response.ok ? '✅ RUNNING' : '❌ NOT ACCESSIBLE'));
  } catch (error) {
